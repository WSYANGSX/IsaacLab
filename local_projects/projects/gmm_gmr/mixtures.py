...
class GMM_GMR(object):
    ...
    def __init__(self, trajectories, pca_components):
        """
        ...
        self.N = self.trajectories.shape[0]  # n samples
        self.T = self.trajectories.shape[1]  # time steps
        self.D = self.trajectories.shape[2]  # data dims

        self.pca = PCA(pca_components)

    def fit(self):
        trajectories_latent = self.pca.fit_transform(
            self.trajectories.reshape(-1, self.D)
        )
        print(f"Explained variance: {np.sum(self.pca.explained_variance_ratio_)}%")
        ...
        for c in components:
            ...
            gmm.fit(temporal_spatio)
            bics.append(gmm.bic(temporal_spatio))

        c = components[np.argmin(bics)]
        print(f"Selected n mixtures: {c}")

        self.gmm = GaussianMixture(n_components=c)
        self.gmm.fit(temporal_spatio)

        print("GMM is converged: ", self.gmm.converged_)
    ...
    def generate_trajectory(self, interval=0.1):
        times = np.arange(
            min(self.centers_temporal), max(self.centers_temporal) + interval, interval
        )
        trj = []
