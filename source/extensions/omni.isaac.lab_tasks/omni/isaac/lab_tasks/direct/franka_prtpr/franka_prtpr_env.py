...
@configclass
class FrankaPrtprEnvCfg(DirectRLEnvCfg):
    # env
...
class FrankaPrtprEnv(DirectRLEnv):
    ...
    cfg: FrankaPrtprEnvCfg

    def __init__(
        self, cfg: FrankaPrtprEnvCfg, render_mode: str | None = None, **kwargs
    ):
        ...
        self.hand_frame = VisualizationMarkers(self.cfg.target)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        self.actions[:, :7] = actions.clone().clamp(-1.0, 1.0)
        targets = (
    ...
    def _reset_target_pose(self, env_ids):
        ...
        new_rot = random_orientation(len(env_ids), device=self.device)
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat(
            (
                ...
                rot_dist,
                arm_dof_pos_scaled,
                self._robot.data.joint_vel[:, :7] * self.cfg.dof_velocity_scale,
            ),
            dim=-1,
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.robot_hand_rot = self._robot.data.body_quat_w[env_ids, self.hand_link_idx]

        self.hand_frame.visualize(
            self._robot.data.body_pos_w[env_ids, self.hand_link_idx],
            self.robot_hand_rot,
        )
    ...
    def _compute_rewards(
        ...
        franka_hand_pos: torch.Tensor,
        franka_hand_rot: torch.Tensor,
        ...
        # distance from hand to the target
        target_dist = torch.norm(franka_hand_pos - target_pos, p=2, dim=-1)
        rot_dist = rotation_distance(franka_hand_rot, target_rot)

        dist_rew = target_dist * dist_reward_scale
        rot_rew = 1.0 / (torch.abs(rot_dist) + rot_eps) * rot_reward_scale

        # regularization on the actions (summed for each environment)
        action_rew = torch.sum(actions**2, dim=-1) * action_penalty_scale

        # Total reward is: position distance + orientation alignment + action regularization + success bonus + fall penalty
        reward = dist_rew + rot_rew + action_rew
