...
class PrtprEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: PrtprEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.target_pos_l = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        self.target_rot_l = torch.zeros(
            (self.num_envs, 4), dtype=torch.float, device=self.device
        )
        self.target_pos_w = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        self.target_rot_w = torch.zeros(
            (self.num_envs, 4), dtype=torch.float, device=self.device
        )

        # default cube positions
        self.cube_pos_l = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        self.cube_rot_l = torch.zeros(
            (self.num_envs, 4), dtype=torch.float, device=self.device
        )
        self.cube_pos_w = torch.zeros(
            (self.num_envs, 3), dtype=torch.float, device=self.device
        )
        self.cube_rot_w = torch.zeros(
            (self.num_envs, 4), dtype=torch.float, device=self.device
        )

        # rot axis
        self.rot_diff = torch.zeros(
            (self.num_envs, 4), dtype=torch.float, device=self.device
        )
        self.rot_axis = torch.zeros(
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.current_target[:, :3] = self._cube.data.root_pos_w + torch.stack(
            (_x, _y, _z), dim=-1
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.cube_pos_l[env_ids] = (
            self.cube_pos_w[env_ids] - self.scene.env_origins[env_ids]
        )
        self.cube_rot_l[env_ids] = self.cube_rot_w[env_ids]

        self.rot_diff[env_ids] = normalize(
            quat_mul(
                self.target_rot_w[env_ids], quat_conjugate(self.cube_rot_w[env_ids])
            )
        )
