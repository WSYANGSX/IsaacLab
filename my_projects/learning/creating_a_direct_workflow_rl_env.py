...
class CartpoleEnvCfg(DirectRLEnvCfg):
    ...
    # robot
    robot: ArticulationCfg = CARTPOLE_CFG.copy().replace(
...
class CartpoleEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CartpoleEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        )  # super.__init__中调用了set_up_scene()
    ...
    def _setup_scene(self):
        self.cartpole = Articulation(self.cfg.robot)
        # add ground plane
        spawn_ground_plane(prim_path="/World/ground", cfg=GroundPlaneCfg())
        # clone, filter, and replicate
        self.scene.clone_environments(
            copy_from_source=False
        )  # from source是从原始prim进行复制，不反映改变
        self.scene.filter_collisions(global_prim_paths=[])
        # add articulation to scene
        self.scene.articulations["cartpole"] = self.cartpole
        # add lights
        light_cfg = sim_untils.DomeLightCfg(intensity=2000.0, color=(0.75, 0.75, 0.75))
        light_cfg.func("/World/Light", light_cfg)

    def _pre_physics_step(self, actions: torch.Tensor):
        self.actions = actions * self.action_scale
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        super()._reset_idx(env_ids)  # reset envs based on env_ids
        ...
        self.cartpole.write_root_state_to_sim(default_root_state[:, :7], env_ids)
...
def compute_rewards(
    ...
    rew_cart_vel = rew_scale_cart_vel * torch.sum(
        torch.square(cart_vel).unsqueeze(dim=1), dim=-1
    )
