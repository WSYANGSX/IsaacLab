...
with open(
    "/home/yangxf/Ominverse_RL_platform/IsaacLab/my_project/rl_projects/gmm_gmr/data/ee_pose3.txt",
    "r",
) as f2:
    ee_pos3 = np.loadtxt(f2)
    ee_pos3 = np.array(ee_pos3)[:, :3]

with open(
    "/home/yangxf/Ominverse_RL_platform/IsaacLab/my_project/rl_projects/gmm_gmr/data/ee_pose4.txt",
    "r",
) as f2:
    ee_pos4 = np.loadtxt(f2)
    ee_pos4 = np.array(ee_pos4)[:, :3]

# with open("rot_data.txt", "r") as f3:
#     rotdist1 = np.loadtxt(f3)
#     rotdist1 = np.array(rotdist1)

# with open("rot_data_1.txt", "r") as f4:
#     rotdist2 = np.loadtxt(f4)
#     rotdist2 = np.array(rotdist2)
...
axe1.plot3D(x2, y2, z2, color="red", label="target position")
axe1.plot3D(x3, y3, z3, color="blue", label="ee position")
axe1.plot3D(x4, y4, z4, color="red", label="target position")
