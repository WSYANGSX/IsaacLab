...
def make_demonstrations(
    ...
    ground_truth[0] = T
    ground_truth[1] = T / 20 + 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(
        -0.5 * ((T - mu) / sigma) ** 2
    )
    ...
    ground_truth = (
        (ground_truth.T - current_start) * amplitude / current_amplitude + start
    ).T
    ...
    for demo_idx in range(n_demonstrations):
        ...
        current_amplitude = current_goal - current_start
        X[:, :, demo_idx] = (
            (X[:, :, demo_idx].T - current_start) * amplitude / current_amplitude
            + start
        ).T
...
X, _ = make_demonstrations(
    n_demonstrations=10, n_steps=50, goal=np.array([1.0, 2.0]), random_state=0
)
X = X.transpose(2, 1, 0)
